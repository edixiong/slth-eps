...
def main():
    ...
    optimizer1 = optim.SGD(
        [p for n, p in model.named_parameters() if p.requires_grad and "score" in n],
        lr=args.lr2,
    ...
    for epoch in range(args.epochs):
        train(model, device, data.train_loader, optimizer1, criterion, epoch+1, args.log_interval)
    ...
    print_model(model) ##
    print('\nTotal time spent pruning {:.2f} minutes'.format(total_time))
    # =======================================

    # Second time training =============
    # no need to perturb if coef = 0
    if args.coef == 0:
        args.epochs_tmp = 0

    optimizer2 = optim.SGD(
        [p for n, p in model.named_parameters() if p.requires_grad and "weight" in n],
        lr=args.lr1,
        momentum=args.momentum,
        weight_decay=args.wd,
    )
    ...
    criterion = nn.CrossEntropyLoss().to(device)
    scheduler2 = StepLR(optimizer2, 50)

    test_acc_arr = np.array([np.nan] * args.epochs_tmp)

    start = time.time()
    for epoch in range(args.epochs_tmp):
        train(model, device, data.train_loader, optimizer2, criterion, epoch+1, args.log_interval)
        scheduler2.step()
        project(args, model, model_copy)
        test_acc_arr[epoch] = test(model, device, data.test_loader, criterion, args.batch_size)
    end = time.time()
    total_time = (end - start) / 60
    print_model(model)
    num_weights = sum(x.numel() for x in model.parameters() if x.requires_grad)     # This calcuation does not include the number of weights in convolutional layers, including baseline models, since we are interested in observing the number of parameters in fully connected layers only. Convolutional layers are randomly initialized and never trained/pruned in any of the models. Note that num_weights is equal to the number of parameters that are being updated in the network
    print('\nTotal time spent training: {:.2f} minutes'.format(total_time))
    # ===================
...
def project(args, model, model_copy):
    for (n1, p1), (n2, p2) in zip(model.named_parameters(), model_copy.named_parameters()):
        assert(n1 == n2)
        if ("weight" in n1):
            p1.data = torch.maximum(torch.minimum(p1.data, p2.data + args.coef), p2.data - args.coef)

def perturb(args, model, model_copy):
    # freeze weight
