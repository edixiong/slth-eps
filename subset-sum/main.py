...
def main():
    ...
    parser.add_argument('--p_c', type=float, default=0, metavar='PC',
                        help='perturbation coefs')
    parser.add_argument('--n', type=int, default=20, metavar='N',
                        help='num of overparametrizatio')
    ...
    for h in keys:
        ...
        for i in tqdm(range(len(approx_model_dict[h]))):
            ...
            if len(approx_model_dict[h][i].size()) == 0:
                w_approx, num_used = utils.FindSubset(w, a, args.epsilon, n, p, check_w_lt_eps=args.check_w_lt_eps)
                approx_model_dict[h][i] = w_approx
                num_weights_approx_remaining += num_used
            else:
                for j in range(len(w)):
                    w_approx, num_used = utils.FindSubset(w[j], a, args.epsilon, n, p, check_w_lt_eps=args.check_w_lt_eps)
