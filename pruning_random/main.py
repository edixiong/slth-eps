...
#     model_copy = copy.deepcopy(model)
    ...
    random_perturb(args, model)
    print("Printing random perturbed model...")
    print_model(model)
    model.reset()
    model.cuda(device)
...
def random_perturb(args, model):
    # freeze weight
    if args.freeze_weights: 
        freeze_model_weights(model)
    
    for n, p in model.named_parameters():
        p.data = p.data + args.coef*(torch.rand(1).cuda(get_device(args)) - 0.5)
